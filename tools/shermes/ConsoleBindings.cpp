...
[[noreturn]] static void throwTypeError(
    ...
    const char *msg) {
  auto typeErrorCtor =
      rt.global().getPropertyAsFunction(rt, "TypeError");
  auto errObj =
      typeErrorCtor.callAsConstructor(rt, facebook::jsi::String::createFromAscii(rt, msg))
          .asObject(rt);
      ...
      jsi::Function::createFromHostFunction(
          ...
          [](jsi::Runtime &rt,
            ...
            if (count < 1 || !args[0].isString())
              throwTypeError(
                  rt, "loadFile requires a string path argument");
            ...
            auto arrayBuf = std::make_shared<VectorMutableBuffer>(
                std::move(data));
            ...
            if (obj.isArrayBuffer(rt)) {
              ...
              if (!bufProp.isObject() ||
                ...
                throwTypeError(
                    ...
                    "loadHBC requires a Uint8Array or ArrayBuffer argument");
              jsi::ArrayBuffer ab =
                  bufProp.asObject(rt).getArrayBuffer(rt);
          ...
          [ctx](jsi::Runtime &rt,
            ...
            for (int i = 0; i < ctx->scriptArgc; ++i) {
              arr.setValueAtIndex(
                  rt,
                  i,
                  jsi::String::createFromAscii(rt, ctx->scriptArgv[i]));
            }
            return arr;
          }));
