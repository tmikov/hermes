...
//! driver (`ast-dump`, `json-parse-dump`, `preparse-dump`).
//!
//! Those drivers live in the unpublished `tools` crate, not in `parser`, so
//! Cargo's `CARGO_BIN_EXE_<name>` — which is only defined for binaries of the
//! *same* package — is not available here. `tools_bin` restores both halves of
//! what that env var gave us:
//!
...
pub fn tools_bin(name: &str) -> PathBuf {
    ...
    // The workspace manifest, reached from `<repo>/rust/crates/parser`.
