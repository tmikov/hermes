...
//! cpp:2159-2171) declares the names in the function/global scope while the
    ...
    ScopedFunctionPromoter<'ast, '_, '_, 'd, 'sc, 'sm, 'tb>
{
    ...
    fn run(
        ...
        if matches!(func_node, Node::Program(_)) {
            ...
            // (cpp:1934's `if (blockBody)`; a `Program` took the branch
    ...
    /// `SemanticResolver::extract_idents_from_decl` (cpp:2292-2382,
