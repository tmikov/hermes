...
// Import of `test/Sema/flow/match-implicit-return.js`, the lit test added by
// upstream `653e49c60` ("Handle Flow match in CheckImplicitReturn"), verbatim
// below this note.
//
// It lives in the PARSER corpus because its own RUN line asks for
// `-Xcompile=false` — the `compile = false` resolution this corpus's oracle
// pair (`sema-parser-dump` vs `sema-dump --parser-entry`) already is, so the
// flag is supplied by the pair rather than by the FLAGS line. Measured: this
// pair's stdout is byte-identical to `hermesc -Xcompile=false -dump-sema
// -parse-flow -Xparse-flow-match` (11615 B, exit 0), so nothing is lost by
// the substitution.
//
// Compiling a match is rejected in SemanticResolver (`90f4a3ac6`,
// `sema_corpus/flow-match-unsupported.js`), which is why parser mode is the
// only mode that reaches `CheckImplicitReturn`'s MatchStatement arm at all —
// and why the driver corpus can never pin it.
//
// `-Xparse-flow-match` is spelled `--Xparse-flow-match` (double dash) for the
// reason `sema_corpus/flow-match-pattern-binding-error.js` gives; the C++
// tool learned both spellings alongside this import.
/**
