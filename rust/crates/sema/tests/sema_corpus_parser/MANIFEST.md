...
`594e9c6a1`** (#2131). It landed as a *restructure*: upstream inlined the
finalizer half into `checkTerminationTryStatement` instead of adding a
`checkTerminationFinalizer` helper, so that helper does not exist in the C++
tree — the Rust port keeps its own and is unchanged. Per this file's history
convention the dated sections below still say "not in `static_h`" and still
name `5ae5260c8`; that is the state at their date. The live table's row is
synced. This corpus, still **17 (9)** against upstream's rewritten binary, is
the evidence that the two C++ forms decide identically.
...
| `implicit-return-try-catch-finally.js` | `test/Sema/implicit-return-try-catch-finally.js` verbatim — the lit test upstream `5ae5260c8` added, now in `static_h` as part of `594e9c6a1`, flagless. **Upstream sync task 4** — see the section of that name at the end of this file. Upstream's file carries TWO RUN lines over ONE set of `CHECK` lines (`-dump-sema` and `-Xcompile=false -dump-sema`), so the same file is imported into BOTH corpora: the driver corpus pins the answers the resolver's split produces, this one pins the answers the UNSPLIT statement produces. The only pin anywhere for the three decisions `5ae5260c8`/`594e9c6a1` adds — under `compile = true` a `TryStatement` never reaches the checker with both children. Verified byte-identical: 7322 B stdout both sides, empty stderr both sides, exit 0 both sides — an oracle-success file. Also measured, not assumed, that this corpus's oracle really is upstream's second RUN line: `hermesc -Xcompile=false -dump-sema` and `sema-parser-dump` produce byte-identical stdout on it (7322 B, exit 0), and its 5 `mayReachImplicitReturn` / 2 `noImplicitReturn` tokens are the same ones the compile path prints |
