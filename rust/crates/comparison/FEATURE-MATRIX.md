...
| **Maturity / ecosystem** | Pre-release (crates.io name reserved, not yet published); React Native lineage via Hermes | Production — powers Next.js compiler, Parcel, Deno transform layer, Prettier (SWC plugin); widely deployed since 2020 | Production — Oxlint 1.0 stable; used in Vite/Rolldown, Prettier 3.6 (experimental plugin); very active development | Production — Biome 2.x; formatter + linter toolchain; used as ESLint/Prettier alternative | Experimental — embeddable JS engine; parser is internal component; not intended for standalone parse use |
| **Rust crate** | `hermes-parser` (reserved; not yet published) | `swc_ecma_parser` | `oxc_parser` | `biome_js_parser` (crates.io publish bug in 0.5.7; unusable as standalone dep) | `boa_parser` |
