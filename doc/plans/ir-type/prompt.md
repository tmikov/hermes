...
2. Read `doc/plans/ir-type/memory.md` for accumulated project knowledge.
3. Read `doc/plans/ir-type/ir-type-system-v2-design.md` for project context and architecture.
4. Read `doc/plans/ir-type/progress.md` to find the current state of all tasks.
...
9. Read `doc/plans/ir-type/phase1-steps.md` to find
   the specification for **your chosen task only** (each step in the plan
...
11. Configure (if needed) and build with Debug+ASan+O1:
    ...
    Run tests as described in `CLAUDE.md`. All builds must succeed
    and all tests must pass.

## Finish

12. Update `doc/plans/ir-type/progress.md`:
...
13. Update `doc/plans/ir-type/memory.md` with anything future sessions should
