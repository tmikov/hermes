...
class IRTypeContext {
 public:
  ...
  Type createClassInstance(uint32_t classId);
  ...
  uint32_t getClassId(Type t) const;
  Type getArrayElementType(Type t) const;
  const FunctionParam *getFunctionParams(Type t, uint16_t &count) const;
  // ... etc.

  // --- Class hierarchy ---
  uint32_t registerClass(uint32_t superClassId);
  bool isSubclass(uint32_t classA, uint32_t classB) const;
...
- `ClassA <: ClassB` iff A is a subclass of B.
...
```cpp
// Class ID 0 is reserved as the "no class" sentinel.
static constexpr uint32_t kNoClassId = 0;

struct ClassInfo {
  uint32_t classId;        // Unique identity (1-based index in classes_ vector)
  uint32_t superClassId;   // Parent class ID (kNoClassId = no parent)
};
```
...
```cpp
// IRTypeContext constructor reserves index 0:
IRTypeContext::IRTypeContext() {
  classes_.push_back({kNoClassId, kNoClassId});  // sentinel at index 0
  // ...
}

uint32_t IRTypeContext::registerClass(uint32_t superClassId) {
  uint32_t id = classes_.size();  // 1-based (index 0 is sentinel)
  classes_.push_back({id, superClassId});
  return id;
}
```
...
```cpp
bool IRTypeContext::isSubclass(uint32_t classA, uint32_t classB) const {
  assert(classA != kNoClassId && classB != kNoClassId);
  uint32_t cur = classA;
  while (cur != kNoClassId) {
    if (cur == classB)
      return true;
    cur = classes_[cur].superClassId;
  }
  return false;
}
```
...
assert(classId != kNoClassId && classId < classes_.size()
    && "invalid class ID");
```
