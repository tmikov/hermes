...
Classes are nominal — identity is a unique `classId`, not structure.

```
ClassInfo:
  classId      : uint32            -- unique identity
  superClassId : uint32            -- parent class (0 if none)
  fields       : [(name, Type)]    -- direct fields
```

**Subtyping**: `ClassInstance(A) <: ClassInstance(B)` iff B is an ancestor
of A in the inheritance chain. The chain is stored in the type context and
consulted during subset checks.
