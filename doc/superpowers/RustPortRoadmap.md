...
>
> **Next: JSX, then the Pre/Lazy passes.** juno has no parser to crib from (`hparser` is FFI-to-C++); port the C++ `JSParserImpl-jsx.cpp`
> (505 lines; the lexer's JSX support is already done) directly, behind the `parse_jsx` flag that already exists (set it from `ast-dump`
> + the differential, wire the lexer JSX contexts). Then the Pre/Lazy passes. Write each phase plan just-in-time (lexer/P1–P7-style) and
> execute subagent-driven.
