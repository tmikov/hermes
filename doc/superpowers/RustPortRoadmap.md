...
**Gate (live, green):** `REQUIRE_DIFFERENTIAL=1 cargo test --manifest-path rust/Cargo.toml -p sema --features dump-bin --test sema_differential -- --nocapture` → "sema differential (tests/sema_corpus): **160 corpus files matched (88 succeeded on hermesc)**" (69 at the end of S1).

**S3-and-later carry-items (from the S1 + S2 reviews):**
- **S3 — `ScopedFunctionPromoter`:** loose block-nested function declarations still assert in TWO places (`resolver/mod.rs:1395` in `visit_program`, `resolver/functions.rs:1057` in `visit_function_body_after_params_visited`) rather than silently skipping promotion, plus `promotedFunctionDecls_`. Three of the eight remaining Deferred corpus rows are blocked on it (`break-in-nested-func.js`, `function-redeclaration-error.js`, `regress-function-promotion-decl.js`).
- **S4 — modules & flavors:** the module visits, CommonJS wrapping, §3.4 rewrite #4 (anonymous `export default function`) and the `$SHBuiltin.moduleFactory`/`export`/`import` branches (all still panic loudly, phase-tagged; `xmod-errors.js` is the deferred row); `FunctionInfo::imports` backref fixup is owed here (reuse S1 T7's `hoisted_functions` pattern); `Decl::Kind::Import` is the one reachable decl kind with no corpus exercise; `resolve_ast_for_parser`. **Flavors:** the typed dialects + the 178 `test/Sema/flow/**` files, and the per-file-flag harness debt they share — `sema_differential.rs` passes no per-file flags, which currently keeps three behaviors out of the corpus (`-enable-eval=false` for `eval() is disabled at runtime`, `-parse-flow` for `typecast not allowed in this context` and `'this' parameter requires typed mode`, `-lazy`) plus the `-fno-std-globals` ambient-decl path; a `// FLAGS:` convention would retire those notes.
- **S5 — lazy + `eval` entry points:** `resolve_ast_lazy`/`resolve_ast_in_scope`, and `visitProgram`'s unported `SaveAndRestore` of `globalScope_` (cpp:216-217), which only becomes observable once `Program` can recur.
- **Regex validation** (`Invalid regular expression: …`) needs the **regex engine** (`lib/Regex/`) — its own future component, not a Sema phase.
- **Documented landmines carried by the corpus, not bugs to chase:** (i) same-location diagnostic ties are unfixable by construction — C++ sorts with an unstable `std::sort`, we sort stably (`invalid-args-eval.js`, one pair at `89:9`); (ii) **hermesc itself aborts** on `class C { x = class {}; }` (`SemContext.cpp:478`, witnessed upstream by `test/hermes/computed-fn-name.js:71`) and asserts on `$SHBuiltin.#x()` (`cast<IdentifierNode>` at cpp:1167) — both pre-existing C++ defects, reproduced faithfully and kept out of the corpus; (iii) `Decl::Special::Eval` is a dead enumerator nothing in the C++ tree ever sets or reads.
...
**Next: S3 — `ScopedFunctionPromoter`** (`lib/Sema/ScopedFunctionPromoter.{h,cpp}`, 37+328 lines): loose-mode block-nested function promotion + `promotedFunctionDecls_`, replacing the two asserts named above. **The S3 plan is WRITTEN and committed — execute it, don't re-plan:** `plans/2026-07-29-sema-s3-scoped-function-promoter.md` (4 tasks, subagent-driven). Spec: `specs/2026-07-26-sema-untyped-design.md`; prior plans: `plans/2026-07-26-sema-s0-foundations.md`, `plans/2026-07-28-sema-s1-declarations-scopes.md`, `plans/2026-07-28-sema-s2-rest-of-walk.md`. |
| IR / IRGen | — | future |
| Optimizer | — | future |
...
> subagent-driven.
>
> **Sema S0 (foundations, 2026-07-26), S1 (declarations & scopes, 2026-07-28) and S2 (rest of the walk, 2026-07-28) are DONE.**
> See the roadmap's Sema row above for what shipped, the gate command (160 corpus files, 88 succeeding on hermesc) and the
> S3/S4/S5 carry-items. **Next: S3 — `ScopedFunctionPromoter`**; plan committed at `plans/2026-07-29-sema-s3-scoped-function-promoter.md` — execute it.
