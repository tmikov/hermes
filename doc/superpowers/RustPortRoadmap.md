...
- **S5 — lazy + `eval` entry points:** `resolve_ast_lazy`/`resolve_ast_in_scope`, and `visitProgram`'s unported `SaveAndRestore` of `globalScope_` (cpp:227), which only becomes observable once `Program` can recur.
