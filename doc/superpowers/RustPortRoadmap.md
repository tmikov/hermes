...
**Gate (live, green):** `REQUIRE_DIFFERENTIAL=1 cargo test --manifest-path rust/Cargo.toml -p sema --features dump-bin --test sema_differential -- --nocapture` → "sema differential (tests/sema_corpus): **172 corpus files matched (96 succeeded on hermesc)**" (160 at the end of S2).
...
> (172 corpus files, 96 succeeding on hermesc) and the S4/S5 carry-items. **Next: S4 — modules & flavors; no plan exists
