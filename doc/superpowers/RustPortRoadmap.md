...
>
> **Next: the Pre/Lazy passes** (the `pass_`/PreParse/Lazy machinery — `SaveFunctionState`, the lazy-function deferral that the eager Full
> pass currently no-ops; the `_param_yield`/`_param_await` args threaded into `parse_function_body` are the dormant hooks). After that the
> Parser component is complete and Sema (scope resolution + FlowChecker) is next. Write each phase plan just-in-time (lexer/P1–P8-style) and
