...
// The GetByIndex twin of recompile-getval-demote-budget0.js. Demotion
// when no recompile can ever happen (spec: "The demotion rule": "When no
// recompile can ever happen ... every observed site is
// demotion-eligible"). load()'s ByIndex site sees only Int32Array targets
// -- learnable evidence, in principle actionable by a recompile that adds
// the typed-array LOAD tier -- but with the recompile budget forced to 0,
// `actionablePossible` is false from the first crossing on, so
...
// tier before any site has a chance to go stable. From then on every read
// hits the inline typed-array fast path directly -- nothing left to
...
ta[0] = 1234;
drive(ta, 300);
// A fresh top-level call: the value read must be the same 1234 whether it
// came back through the demoted plain helper (BUDGET0) or the inline
// typed-array LOAD tier (DEFAULT).
print(load(ta));
