...
// The GetByIndex twin of recompile-getval-duo.js. ONE GetByIndex site --
// one literal index, in one function -- sees both a dense array and an
// Int32Array, interleaved. The array reads always hit the JSArray tier (a
// non-hole element of a dense array) and decline never, so they leave NO
// trace in the record at all; the Int32Array reads always miss that tier's
// kind guard and decline through the helper, which records a mono
// typed-array kind. Only the Int32Array declines drive the shared counter,
// so the loop must give them, alone, at least 64 declines to cross the
// threshold (64, pinned on the RUN line) -- 70 each, interleaved, does
// that with margin.
//
...
//
// The key is a LITERAL uint8: ISel.cpp lowers exactly that to GetByIndex,
// and a variable key would lower to GetByVal and pin the other opcode's
// chaining instead. The `// getByIdx r` dump comment is the pin for that.
function f(x) {
  return x[2];
}
var arr = [5, 6, 7, 8];
