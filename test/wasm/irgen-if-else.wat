;; Copyright (c) Meta Platforms, Inc. and affiliates.
;;
;; This source code is licensed under the MIT license found in the
;; LICENSE file in the root directory of this source tree.

;; Test if/else generates correct IR.

;; REQUIRES: wasm
;; RUN: %wat2wasm %s -o %t.wasm
;; RUN: %hermesc --wasm --dump-ir -O0 %t.wasm | %FileCheck %s

;; Test 1: if/else with result, true condition → 42
;; CHECK-LABEL: function wasm_func_0(): any
;; CHECK:         CondBranchInst 1: number, %BB2, %BB3
;; CHECK:       %BB2:
;; CHECK-NEXT:    BranchInst %BB4
;; CHECK:       %BB3:
;; CHECK-NEXT:    BranchInst %BB4
;; CHECK:       %BB4:
;; CHECK-NEXT:    %{{[0-9]+}} = PhiInst (:number) 42: number, %BB2, 99: number, %BB3

;; Test 2: if/else with result, false condition → 99
;; CHECK-LABEL: function wasm_func_1(): any
;; CHECK:         CondBranchInst 0: number, %BB2, %BB3
;; CHECK:       %BB2:
;; CHECK-NEXT:    BranchInst %BB4
;; CHECK:       %BB3:
;; CHECK-NEXT:    BranchInst %BB4
;; CHECK:       %BB4:
;; CHECK-NEXT:    %{{[0-9]+}} = PhiInst (:number) 42: number, %BB2, 99: number, %BB3

;; Test 3: if without else, void body
;; CHECK-LABEL: function wasm_func_2(p0: any): any
;; CHECK:         CondBranchInst %{{[0-9]+}}: any, %BB2, %BB3
;; CHECK:       %BB2:
;; CHECK-NEXT:    StoreStackInst 20: number, %{{[0-9]+}}: any
;; CHECK-NEXT:    BranchInst %BB4
;; CHECK:       %BB3:
;; CHECK-NEXT:    BranchInst %BB4

;; Test 4: nested if/else
;; CHECK-LABEL: function wasm_func_3(p0: any, p1: any): any
;; CHECK:         CondBranchInst %{{[0-9]+}}: any, %BB2, %BB3
;; CHECK:       %BB2:
;; CHECK:         CondBranchInst %{{[0-9]+}}: any, %BB5, %BB6
;; CHECK:       %BB3:
;; CHECK-NEXT:    BranchInst %BB4
;; CHECK:       %BB4:
;; CHECK-NEXT:    %{{[0-9]+}} = PhiInst (:number) %{{[0-9]+}}: number, %BB7, 3: number, %BB3
;; CHECK:       %BB5:
;; CHECK-NEXT:    BranchInst %BB7
;; CHECK:       %BB6:
;; CHECK-NEXT:    BranchInst %BB7
;; CHECK:       %BB7:
;; CHECK-NEXT:    %{{[0-9]+}} = PhiInst (:number) 1: number, %BB5, 2: number, %BB6

(module
  (func $if_true (export "if_true") (result i32)
    (if (result i32) (i32.const 1)
      (then (i32.const 42))
      (else (i32.const 99))))

  (func $if_false (export "if_false") (result i32)
    (if (result i32) (i32.const 0)
      (then (i32.const 42))
      (else (i32.const 99))))

  (func $if_void (export "if_void") (param i32) (result i32)
    (local i32)
    (i32.const 10)
    (local.set 1)
    (if (local.get 0)
      (then
        (i32.const 20)
        (local.set 1)))
    (local.get 1))

  (func $nested (export "nested") (param i32) (param i32) (result i32)
    (if (result i32) (local.get 0)
      (then
        (if (result i32) (local.get 1)
          (then (i32.const 1))
          (else (i32.const 2))))
      (else (i32.const 3))))
)
