...
;; RUN: %wat2wasm %S/e2e-table-import-advanced-exporter.wat_ -o %t-exporter.wasm && %hermesc --wasm -emit-binary -out %t-exporter.hbc %t-exporter.wasm && %wat2wasm %s -o %t.wasm && %hermesc --wasm -emit-binary -out %t.hbc %t.wasm && %hermes -Xhermes-internal-test-methods -Xenable-untrusted-bytecode-from-js %S/e2e-table-import-advanced-driver.js_ -- %t-exporter.hbc %t.hbc | %FileCheck --match-full-lines %s
