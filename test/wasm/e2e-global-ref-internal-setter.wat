...
;; which initializes a missing property map -- and it sits between the
;; builtin's entry and both its stores. The loop at the end allocates between
;; writes. In a build without HERMESVM_SANITIZE_HANDLES the flag is ignored
;; and this is an ordinary behavioural test.
...
;; CHECK-NEXT: put_fn undefined: TypeError: Wasm global.set: a funcref global requires null or a WebAssembly exported function
...
;; CHECK-NEXT: put_jsfn a plain function: TypeError: Wasm global.set: a funcref global requires null or a WebAssembly exported function
